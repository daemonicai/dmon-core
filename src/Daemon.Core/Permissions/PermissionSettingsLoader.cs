...
/// <summary>
/// Loads and saves permission settings from .daemon/settings.yaml (project) or
/// ~/.daemon/settings.yaml (global). Uses a minimal line-based YAML parser that
...
public sealed class PermissionSettingsLoader : IPermissionSettings
{
    ...
    {
        string path = Path.Combine(workingDirectory, ".daemon", "settings.yaml");
    ...
    public static PermissionSettingsLoader LoadGlobal()
    {
        ...
        string path = Path.Combine(home, ".daemon", "settings.yaml");
        return new PermissionSettingsLoader(path, Load(path));
    }
