...
/// The client's own device credential, presented to `Dmon.Network` as an
...
/// D13/D16-carve): it holds a credential and computes its hash, but it
/// neither generates one nor persists one — where the credential comes
...
public struct DeviceCredential: Sendable {
    ...
    /// (design risk 2). See `DeviceCredentialTests` for the pinned digests
    /// this must continue to reproduce.
    public var secretHash: String {
        Self.secretHash(ofToken: secret)
    }

    /// Static so device-credential-store code (the host-facing half of
    /// task 6.5) can hash a bare token — e.g. when self-provisioning a new
    /// entry into `devices.json` — without constructing a full credential
...
extension DeviceCredential: CustomStringConvertible, CustomDebugStringConvertible {
    /// Redacts `secret`. `GatewayConnection` logs errors with
    /// `String(describing: error)|(reflecting:)`, and any error type that
    /// ever carries a credential must not leak the secret through that
    /// path — the key id alone is enough to identify which credential was
    /// in play.
    public var description: String {
        "DeviceCredential(keyId: \"\(keyId)\", secret: <redacted>)"
    }
