...
/// Where this host's own `DeviceKeySecret` comes from.
///
...
/// secret and computes its hash, but never decides where one comes from. A
/// `DeviceKeySecretStore` conformer may assume a filesystem, and in
/// `KeychainDeviceKeySecretStore`'s case a Keychain, shared with the `Dmon.Network` host
...
/// to prove it.
///
/// The read path (`load()`) defines only the operation it needs: loading a
/// secret a prior run already holds. `store(_:)` is the write path's addition
/// (`DeviceKeyProvisioner`) — persisting a freshly generated secret so a later
/// `load()` call finds it.
///
/// **Memory zeroing, considered and declined.** `DeviceKeySecret.secret` is a plain
...
///   (`"Bearer \(secret.secret)"`), so there is no lower-level byte buffer this type
...
///   runs as the same user account that owns `devices.json`, and reads this secret
...
public protocol DeviceKeySecretStore: Sendable {
    /// This host's own device key secret, or `nil` if none has been provisioned yet.
    func load() async throws -> DeviceKeySecret?

    /// Persists `secret` as this host's own device key secret. Called exactly once per
