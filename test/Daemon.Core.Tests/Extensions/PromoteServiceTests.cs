...
public sealed class PromoteServiceTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), $"daemon-promote-test-{Guid.NewGuid():N}");
    ...
    public async Task PromoteAsync_CsprojContainsRequiredReferences()
    {
        ...
        Assert.Contains("Daemon.Extensions", csprojContent);
    }

    [Fact]
    public async Task PromoteAsync_GeneratedClassImplementsIDaemonExtension()
    {
        ...
        Assert.Contains("IDaemonExtension", classContent);
