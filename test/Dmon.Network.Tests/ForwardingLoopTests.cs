...
/// <summary>
/// Drives the real <see cref="NetworkConnectionEndpoint"/> receive/dispatch loop over an
...
public sealed class ForwardingLoopTests
{
    ...
    public async Task NonStringId_DoesNotThrow_AndIsForwardedRawWithoutAck()
    {
        ...
        RecordingConnection connection = new();
        NetworkConnectionEndpoint endpoint = NewEndpoint();
    ...
    private static NetworkConnectionEndpoint NewEndpoint() =>
    ...
    /// pong is asserted via the <see cref="INetworkConnection"/> instead).
    /// </summary>
