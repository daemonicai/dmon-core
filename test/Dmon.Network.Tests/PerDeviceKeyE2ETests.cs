...
public sealed class PerDeviceKeyE2ETests : IDisposable
{
    ...
    /// Matches the helper in <c>AuthAndBindTests</c> and <c>NetworkIntegrationTests</c>.
    /// </summary>
    ...
    /// <see cref="FakeClientWebSocket"/> in <c>NetworkIntegrationTests</c> sets state to
    ...
    /// <summary>
    /// An <see cref="INetworkConnection"/> with a non-null <see cref="KeyId"/>, backed by
    ...
    /// </summary>
    private sealed class KeyedAbortableConnection(string keyId) : INetworkConnection
    {
