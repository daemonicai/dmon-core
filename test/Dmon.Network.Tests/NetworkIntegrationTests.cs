...
public sealed class NetworkIntegrationTests
{
    ...
    public async Task Reaper_InFlightHandler_SurvivesIdleTtl_ReapedAtAbsoluteMax()
    {
        ...
        IOptionsMonitor<NetworkOptions> options = MakeOptions(new NetworkOptions
        {
    ...
    /// <see cref="INetworkConnection.Abort"/> — then connA's loop (still running in the eviction
    ...
    /// <c>Authorization</c> header, <see cref="NetworkConnectionEndpoint.HandleAsync"/>
    ...
    {
        NetworkConnectionEndpoint endpoint = MakeEndpointWithKey("correct-key");
    ...
    {
        NetworkConnectionEndpoint endpoint = MakeEndpointWithKey(token: null);
        DefaultHttpContext context = new();
    ...
    /// <see cref="NetworkConnectionEndpoint.DriveSessionHandshakeAsync"/> to throw
    ...
    /// <summary>
    /// 11.4b — <see cref="NetworkOptions.CreateHandshakeTimeoutSeconds"/> is surfaced and
    /// defaults to 30, so the option is discoverable and never zero.
    /// </summary>
    [Fact]
    public void NetworkOptions_CreateHandshakeTimeoutSeconds_DefaultsTo30()
    {
    ...
    private static NetworkConnectionEndpoint MakeEndpoint() =>
    ...
    /// </summary>
    private static NetworkConnectionEndpoint MakeEndpointWithKey(string? token)
    {
    ...
    private static IOptionsMonitor<NetworkOptions> MakeOptions(NetworkOptions opts) =>
