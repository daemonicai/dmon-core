...
/// Endpoint tests remain IO-free via the no-op default in <see cref="GatewayConnectionEndpoint.TestOptions"/>.
/// </summary>
public sealed class LastSeenWriterTests : IDisposable
{
    ...
    {
        GatewayOptions opts = new() { LastSeenThrottleSeconds = throttleSeconds };
        StaticOptionsMonitor<GatewayOptions> monitor = new(opts);
    ...
    public void RecordAttach_WriteFailure_DoesNotThrow()
    {
        ...
        GatewayOptions opts = new() { LastSeenThrottleSeconds = 0 };
        StaticOptionsMonitor<GatewayOptions> monitor = new(opts);
