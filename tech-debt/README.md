...
- ~~[Three `Dmon.Terminal.Tests` tests hang, intermittently](terminal-tests-hang.md)~~ — **resolved** by #117 (`9edd380`, 2026-09-11). All six copies of the wait-for-EOF shape, Terminal and Core, now use one shared `test/Shared/ProcessRunner.cs`: node reuse is off, and the post-exit drain is bounded. Forced both ways. The workaround variable is no longer needed. Production's `BuildFileBasedProgramAsync` has the same shape, but it did not reproduce; see the note.
