...
- ~~[Three `Dmon.Terminal.Tests` tests hang, intermittently](terminal-tests-hang.md)~~ — **resolved** on `fix/terminal-tests-hang` (2026-09-11). All six copies of the wait-for-EOF shape, Terminal and Core, now use one shared `test/Shared/ProcessRunner.cs`: node reuse is off, and the post-exit drain is bounded. Forced both ways. The workaround variable is no longer needed. Production's `BuildFileBasedProgramAsync` has the same shape, but it did not reproduce; see the note.
...
- ~~[The live e2e test writes into the user's home session store](live-e2e-test-writes-into-home-session-store.md)~~ — **resolved** by `session-root-resolution` (#116, `90eed64`). There were three writers (the live test, `CoreProcessFixture`, and `CoreProcessManagerRestartTests`); all now pin `sessionStore: local` in a temp-root `config.yaml`. The accumulated test sessions are not yet pruned.
