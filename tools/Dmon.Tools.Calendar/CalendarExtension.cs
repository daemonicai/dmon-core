...
/// <c>list_upcoming_events</c>. Each tool calls the Daemon.Calendar HTTP API.
/// </summary>
/// <remarks>
/// Wire it into a composition root with <c>builder.AddCalendarAbilities()</c>
/// (configured from the <c>DCAL_BASE_URL</c> and <c>DCAL_API_KEY</c> environment
/// variables) or <c>builder.AddToolExtension(new CalendarExtension(baseUrl, apiKey))</c>
/// for explicit configuration.
/// </remarks>
public sealed class CalendarExtension : IToolExtension
{
    ...
    /// <summary>Creates the extension against an explicit Daemon.Calendar endpoint.</summary>
    ...
    public CalendarExtension(string baseUrl, string? apiKey, HttpClient? httpClient = null)
    {
    ...
    public string Description =>
        "Look up calendar events and list upcoming events via the Daemon.Calendar server.";
