...
public sealed class RetryingChatClient : IChatClient
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            using Activity? callActivity = DmonTelemetry.Source.StartActivity("provider.call");
            if (callActivity is not null)
            {
                callActivity.SetTag("dmon.provider", _providerName);
                callActivity.SetTag("dmon.model", _modelName);
                callActivity.SetTag("dmon.retry.attempt", attempt);
            }
            ...
            catch (Exception ex) when (IsRetryable(ex))
            {
                ...
                DmonTelemetry.RecordProviderRetry(_providerName, ex.Message);
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            DmonTelemetry.RecordProviderRetry(_providerName, producerException?.Message ?? "unknown");
