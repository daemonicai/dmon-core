...
The `calendar-sync` capability defines the `Daemon.Calendar` server that backs the `calendar-lookup` tools. It subscribes to an iCal feed, parses and expands events with `Ical.Net`, persists them into a local SQLite store, and exposes an HTTP surface for sync control and health. Lookup tools query this store rather than the model reasoning over raw event lists.

## Requirements

### Requirement: Daemon.Calendar syncs an iCal subscription URL into SQLite on startup
`CalendarSyncService` SHALL perform an immediate sync when `Daemon.Calendar` starts, downloading the iCal URL configured via `DCAL_ICAL_URL`, parsing all events with `Ical.Net`, and persisting them to the local SQLite database. The server SHALL NOT serve lookup requests until the initial sync completes.

#### Scenario: Initial sync populates the database
- **WHEN** `Daemon.Calendar` starts with a valid `DCAL_ICAL_URL`
...
- **WHEN** `Daemon.Calendar` starts without `DCAL_ICAL_URL` configured
- **THEN** the process exits with a descriptive error message

### Requirement: Daemon.Calendar re-syncs periodically
...
### Requirement: Daemon.Calendar expands recurring events over a configurable horizon
