...
Define the standing contract for the `Dmon.Memory.Meko` long-term memory backend (ADR-026): an `ILongTermMemory` implementation over Meko's `memory_*` MCP tools, how dmon's `MemoryScope` maps onto Meko's `run_id` partitioning, the opt-in turn-capture gate, the disabled-tier / flush fallback semantics, and the `AddMekoLongTermMemory(...)` DI registration verb that the `IMemory` facade resolves. All Meko/MCP coupling is confined behind the `ILongTermMemory` interface.
