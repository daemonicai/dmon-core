# ability-registry Specification

## Purpose
A scope-aware, per-turn tool-manifest builder: the C# privacy gate that withholds out-of-scope tools. `AbilityRegistry` groups tools by the scope label their `IAbilityProvider` declares and, via `ForScope`, hands a terminal client only the tools registered for the requested scope — so out-of-scope abilities are never surfaced into a turn's manifest. Ability providers are registered by DI-discovery and are an independent registry from the global `IToolExtension` tool pipeline.
