...
The console host SHALL communicate with the agent core exclusively via the JSONL-over-stdio RPC protocol (ADR-003). No agent logic, session management, or tool execution SHALL be implemented in the host process. The host SHALL be implemented using Spectre.Console for prompts, streaming displays, and markup.
...
- **THEN** the host renders incremental text to the terminal in real time
