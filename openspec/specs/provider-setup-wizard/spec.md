...
The terminal renderer SHALL pattern-match `WizardStep` subtypes onto `InlinePrompt` interactions: `ChooseOneStep`/`ChooseManyStep` to a selection prompt, `TextInputStep` to a line read (honouring `Secret` and `Required`), `YesNoStep` to a confirmation prompt, and `InfoStep` to a message that advances without input. The renderer SHALL translate the underlying back signal and cancel signal into the engine's back and cancel outcomes respectively. When a `TextInputStep` has `Secret = true` and a non-null `Default`, the renderer SHALL display a masked hint (`********`) rather than the raw value.
