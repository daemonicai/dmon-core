# dmon-home Specification

## Purpose

`dmon-home` is the macOS host product: a native, always-on desktop presence for a personal
dmon, living in the repository's `home/` role bucket alongside the .NET buckets rather than
inside them.

This capability governs the *shell* — where the product lives in the repo, how it is built,
and what the application itself is. It fixes the Apple Silicon target, keeps the Xcode project
generated from a manifest rather than hand-edited, and pushes application logic down into local
Swift packages so the app target stays a thin composition root. It also covers what the host
does as a desktop citizen: an unsandboxed bundle that can hold a microphone grant, stream its
children's output where a person can read it, keep the machine awake while it is serving, and
carry a conversation.

What it deliberately does not cover: how the host reaches its agent, which is
`dmon-home-gateway-client`, and how it runs the processes it depends on, which is
`dmon-home-supervision`.
