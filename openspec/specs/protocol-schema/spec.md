...
## Purpose
A published, versioned, machine-readable description of the dmon wire protocol — the `Command` and `Event` hierarchies, the conversation `Part` union, and the gateway `gw` control frames — generated from the live `Dmon.Protocol` types and kept in lock-step with them by a build/test gate, plus a consumer-facing guide so external client authors (e.g. an iOS/Swift client) can build against the gateway without reading C# source.
