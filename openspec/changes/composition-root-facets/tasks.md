...
- [x] 8.1 Publish `Dmon.Tools.Builtin` (references only `Dmon.Abstractions`/`Dmon.Protocol`) exposing `.AddBuiltinTools()`; remove the hard-wired builtin-tools registration from the engine.
- [x] 8.2 Update the scaffolded `Dmon.cs` (`dmon init`) and the prebuilt stock default to `#:package` `dmoncore` + the cloud provider packages + `Dmon.Tools.Builtin` and compose them; confirm a tool-less locked-down composition is valid.
- [x] 8.3 Set protocol-keyed lockstep versions across contracts + first-party packages; confirm an incompatible `#:package` set fails at `dotnet restore`.
- [ ] 8.4 Human-in-the-loop verification (provide a copy-pasteable recipe): build + run the stock default core and a minimal single-provider `Dmon.cs`, confirm `agentReady` and a tool call work; verify a sub-agent web-search-style tool composes with `Action<IProviderRegistration>`. **DEFERRED** — the live provider → tool-call round-trip rides on the planned `host-rpc-abstraction` change as an automated e2e test (`IRpcTransport`/`IRpcClient` make driving the core ergonomic; the terminal currently swallows core stderr, hiding failures). Partial coverage already exists: `BuiltinToolsRegistrationTests` (six tools register / locked-down omits them) + `CoreProcessFixture` (real core reaches `agentReady`). Does not block archive.
- [x] 8.5 Gate: `make build` clean, `make test` green, `openspec validate composition-root-facets --strict`.
