...
- [x] 8.4 Human-in-the-loop verification (provide a copy-pasteable recipe): build + run the stock default core and a minimal single-provider `Dmon.cs`, confirm `agentReady` and a tool call work; verify a sub-agent web-search-style tool composes with `Action<IProviderRegistration>`. **Covered by automated e2e in `host-rpc-abstraction`**: `test/Dmon.Core.Tests/Integration/LiveToolCallE2ETest.cs` launches a real core via `ICoreLauncher`, wraps it in `IRpcClient`/`CoreProcessRpcTransport`, performs session create+load, submits a turn that triggers `read_file`, and asserts `ToolExecutionStartEvent`+`ToolExecutionEndEvent`+`TurnEndEvent`. Skips honestly (never silently passes) when no provider key is configured (ADR-005).
