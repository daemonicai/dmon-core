...
## Live-verification findings (Group 8.3)

Live verification against a real core surfaced several facts that correct assumptions made during earlier groups. Recorded here so the design matches reality (none required a core/protocol/runtime change — the RPC surface held):

- **Live assistant turns are text-only.** The core emits `TurnEndEvent.Message` as `{ role: "assistant", content: "<text>" }` (an anonymous object in `TurnHandler`), **not** a parts-based `MessageRecord`. The desktop settles the assistant turn from that `content` string. The ADR-016 **parts model** (`ToolCallPart`/`ToolResultPart`/`ImagePart`/`ReasoningPart`/`UnknownPart`) and its type-selected templates (Group 5.2) are therefore exercised only by the **history-load path** (a persisted `MessageRecord` rendered via `new MessageViewModel(MessageRecord)`), not by live `TurnEnd`. Live tool activity arrives as separate `ToolExecutionStart/EndEvent`s (rendered as tool presentations is a future affordance). The part-templates remain correct for history; the live streaming surface is text + status events.
- **The Splat→MEDI resolver swap (Decision 7) must fully re-establish ReactiveUI's platform services.** Swapping the resolver bypasses ReactiveUI.Avalonia's normal `UseReactiveUI` initialisation, so the composition root must explicitly re-register the base RxUI services (`ReactiveUIBuilder` `WithCoreServices`/`WithPlatformServices`), the **Avalonia** platform services (`WithAvalonia()` → `IActivationForViewFetcher`/command binding), **and** set the main-thread scheduler to `AvaloniaScheduler.Instance`. Missing any of these fails only at runtime (not in tests), so the bridge is guarded by tests asserting `ICreatesObservableForProperty`, `IActivationForViewFetcher`, and the main-thread scheduler type resolve after the bridge runs. *(A follow-up could harden this by establishing the MEDI bridge before `UseReactiveUI` runs, rather than init-then-swap-then-repopulate.)*
- **UI marshalling and delta coalescing need distinct schedulers.** Driving the `messageDelta` `Buffer` window off the Avalonia dispatcher scheduler does not flush reliably on the live dispatcher; the coalescing timer runs on `DefaultScheduler.Instance` and the flushed batch is marshalled to the UI via `ObserveOn(AvaloniaScheduler)`.
- **Cross-platform fonts only.** `Markdown.Avalonia`'s default code style requested **Consolas** (Windows-only), which hard-crashed the render pass on macOS. The markdown theme pins monospace to `Courier New, Monospace`.
- **Core working directory.** The desktop launches the core without an explicit working directory, so the core resolves its config (`.dmon/config.local.yaml`, active model) relative to the launch cwd — IDE-dependent. Anchoring the core to a deterministic working directory (and an in-app provider/model picker, currently out of scope) is a sensible follow-up.
