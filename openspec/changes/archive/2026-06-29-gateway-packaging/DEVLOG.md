...
## Block 8 — tasks 12.1, 12.2, 12.4 (launch-defect fix — DONE, committed)

**10.4 human-verify FOUND A REAL BUG:** `ndmon` crashed at startup — `System.InvalidOperationException: A suitable constructor for type 'DeviceKeyStoreWatcher' could not be located`. Two PRE-EXISTING latent defects (the host had NEVER actually been launched before this change — the original gap — so the DI graph was never exercised; unit tests construct these types directly via InternalsVisibleTo):

- **12.1 (DI ctor visibility):** MS.DI binds only PUBLIC constructors. `DeviceKeyStoreWatcher` (hosted service) and `LastSeenWriter` (singleton resolved by the endpoint factory) had only `internal` ctors → crash at `Host.StartAsync` / first connection. Fix: `DeviceKeyStoreWatcher` ctor → public (class stays internal sealed); `LastSeenWriter` — made ONLY the `(NetworkDeviceKeyPaths, …)` ctor public, kept the `(string?, …)` ctor + `CreateNoOp()` internal (avoids ambiguous-ctor DI selection; tests reach them via InternalsVisibleTo). **INVARIANT for future architects: any DI-resolved type in this host must keep its DI ctor public — `NetworkHostServicesTests` is now the standing guard.**
- **12.2 (regression test):** extracted Program.cs lines 72–107 registrations into `frontends/Dmon.Network/NetworkHostServices.cs` `AddNetworkHostServices(IServiceCollection, NetworkDeviceKeyPaths, DeviceKeySet)` (Program.cs now CALLS it — no drift). New `NetworkHostServicesTests` builds the provider with `ValidateOnBuild`/`ValidateScopes` AND explicitly resolves `IEnumerable<IHostedService>` + `LastSeenWriter` + `NetworkConnectionEndpoint` (ValidateOnBuild alone does NOT construct hosted/instance/factory services — the explicit resolves are what catch this bug; the test fails if 12.1 is reverted).
- **12.4 (`make network` stale-reinstall):** SECOND defect found during verify — `make network` packed `0.1.0` but `dotnet tool update --version 0.1.0` is a NO-OP when 0.1.0 already installed (pinned version, NuGet treats it immutable), so the STALE binary persisted at `~/.dotnet/tools/ndmon` (violated task 5.1's "update if already installed"). Fix: `network` target now `pack` → `-dotnet tool uninstall` (—ignore-if-absent) → `rm -rf "$(HOME)/.nuget/packages/dmon.network"` (evict cached extraction) → fresh `install`. Idempotent; OOTB-safe; verified twice.

**VERIFIED LIVE (orchestrator):** freshly-built DLL boots; after the `make network` fix the INSTALLED `~/.dotnet/tools/ndmon` boots cleanly — `Device-key store watcher started`, `Now listening on http://127.0.0.1:5500`, `Application started`, NO crash. Both reviewer sub-audits approved. Gates: build 0-warn, Network.Tests 209/209 (208+1 new), Core 592, validate --strict.

**LESSON:** a packaged host needs a real launch smoke / DI-graph validation test, not just unit tests — three of this change's bugs (latent DI crash ×2 + stale reinstall) only surfaced at actual process launch, which no gate exercised until 10.4. The new `NetworkHostServicesTests` closes the DI-graph hole permanently; the live-launch remains the 10.4 human-verify.

## ⏳ 10.4 HUMAN-VERIFY (orchestrator → user): re-run after Block 8 — launch dmonium, confirm Network health row green OOTB with no DMON_NETWORK_PATH. Process-boot already proven by orchestrator; user confirms the dmonium UI row. NOT yet ticked.
