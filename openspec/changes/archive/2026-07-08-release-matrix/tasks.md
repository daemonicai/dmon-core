...
- [~] 6.3 Human-verify recipe (no secret in automation): a copy-pasteable sequence to push one real per-package tag and confirm nuget.org receipt, plus one `app/*` tag and confirm the GitHub Release attachment — to be run by the maintainer, then ticked. **DEFERRED at archive time:** the recipe is authored (`design.md` → "Maintainer verification (task 6.3)"); the live nuget.org receipt is intentionally routed through NuGet Trusted Publishing (OIDC) rather than a long-lived `NUGET_API_KEY`, so the first real publish is performed and verified under the follow-up change `release-trusted-publishing`. All release machinery is dry-run-verified (task 6.2).
