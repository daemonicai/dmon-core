...
| 4 | State layer: `InputReader` | `a068bfd` (+ `9d4a3fb` for §4.5 sign-off) | 96 in `Dmon.Terminal.Tests` (+16 net since Phase 3: new `InputStateLayerTests` 13 covering buffer mirror under lock, history append/drop/whitespace/bounded/eviction-while-locked, IsLocked default+roundtrip; 3 new `ConsoleEventHandlerTests` covering the dispatch-layer locked-drop + unlock re-enables forwarding) | Chunked across 2 worker calls (source + tests). Reviewer approved first audit with 0 blockers + 5 cosmetic nits + 4 architectural notes — no nit fixes applied (the duplicate-lock-check WHY-comment requested by nit 3 was already present in the worker's first pass). `InputReader.cs` renamed `InputStateLayer.cs` (the type is a pure state mirror, not a reader); `Program.cs` lost ~30 lines of `inputTask` / `inputEnum` plumbing (`Task.WhenAny(nextEvent, reloadSignal.Task)` replaces the dual-source `WhenAny`); worker added a `TaskCompletionSource<bool> reloadSignal` deviation that the brief was missing — without it `/reload` would silently hang since input no longer flows through the session loop. §4.5 signed off via the no-LLM recipe (typing, `/reload`, Ctrl+C); full-LLM smoke deferred behind the pre-existing Dmon.Core MCP/M.E.AI binding crash. |
| 5 | `MarkdownRenderer` rewrite + drop Spectre | (this commit) | 118 in `Dmon.Terminal.Tests` (+22 net since Phase 4: 22 new pure-function `MarkdownRendererTests` + 3 new SettleTurn tests + 1 new TurnEnd-ordering test in `ConsoleEventHandlerTests`) | Chunked across 2 worker calls (source rewrite + tests). Reviewer approved first audit with 0 blockers, 4 cosmetic nits, 5 architectural notes — no nit fixes applied (all opportunistic follow-ups). `MarkdownRenderer` is now a pure function `string → IReadOnlyList<Line>` with an internal `LineAccumulator` helper; `TerminalRenderer.SettleTurn` resolves the Phase 1 deferral by calling `SetContent(lines)` before `Commit`; the `spectreMarkup` parameter renamed `markdownSource` (Phase 1 nit closed). `Spectre.Console` package reference removed; transitive graph confirmed clean. Visual deviation up front: code blocks lost the Spectre `grey35` background tint (no exact dcli `AnsiColor` analogue). The dcli `<ProjectReference>` stays in place per user direction — PR-time swap. §5.7 live visual compare blocked behind the same MCP/M.E.AI crash; tier-A tests cover the rendering contract. |
...
- **§4.5 — Phase 4 full end-to-end smoke** (unblocks §1.6, §2.8, §3.6 retroactively).
  - First attempt (full recipe with LLM call) hit a pre-existing Dmon.Core crash on the streaming turn: `Method not found: 'System.String Microsoft.Extensions.AI.HostedMcpServerTool.get_AuthorizationToken()'`. The crash is downstream of Phase 4 (no migration commit touched `Dmon.Core.csproj` or any M.E.AI code; the version was last bumped in `7bbd6d9` long before this branch). Captured as a follow-up below.
  - Second attempt with the no-LLM recipe (`build/dmon` → type/backspace → `/reload` → Ctrl+C) succeeded:
    - dcli input editor accepts typing/backspace correctly.
    - `/reload` end-to-end works: `[Reload] Restarting core…` → core actually restarts → fresh `[Ready] dmon core v1.0.0.0 (protocol 1.0)` from the new process. This validates the worker's `reloadSignal` TCS deviation in production (the session-loop unblock path).
    - Ctrl+C exits cleanly.
  - Status: **signed off 2026-05-28**. The locked-input-during-streaming spec scenario is verified by tier-A tests (`ConsoleEventHandlerTests.HandleAsync_InputSubmitted_WhileLocked_DropsAndDoesNotForward` + `_StateLayerHistoryNotAppended` + `_AfterTurnEnd_ForwardsAgain`); live verification deferred until the MCP crash unblocks it.

## Open follow-ups / known gaps (after this change lands — NOT in scope here)

- **Dmon.Core crash on streaming turn: `HostedMcpServerTool.get_AuthorizationToken` Method not found.** Surfaced during the §4.5 first-attempt smoke; the user typed a plain message, the core started a turn (`[Notice]` + `── Thinking… ──` rendered), then the core process emitted an `[Error] Method not found: 'System.String Microsoft.Extensions.AI.HostedMcpServerTool.get_AuthorizationToken()'` and exited. Diagnosis: a transitive-dependency version skew between `Microsoft.Extensions.AI` 10.6.0 (what `Dmon.Core.csproj` pins) and an older M.E.AI shape baked into one of the publish-time DLLs in `build/` (probably `GeminiDotnet.Extensions.AI.dll`). **Unrelated to dmon-migration** — no commit in this branch touched `Dmon.Core.csproj`, `Dmon.Providers*.csproj`, or any M.E.AI consumer code. Fix likely needs an M.E.AI version pin in `Dmon.Providers` and/or a `GeminiDotnet` update; tackle as a separate change, not in this migration.
- **Missing `── goodbye ──` separator on Ctrl+C exit.** The user's §4.5 smoke output ended at the post-reload `[Ready]` line with no goodbye separator. `Program.cs` calls `renderer.PrintSeparator("goodbye")` after `coreProcess.StopAsync()`. Likely cause: dcli's `ITerminal.DisposeAsync` runs at the `await using` scope exit (which fires before the `PrintSeparator` call) so the separator writes into a torn-down fixed region. Cosmetic — exit is otherwise clean. Move the `PrintSeparator("goodbye")` call before `await coreProcess.StopAsync()` and/or before the `await using` scope exits, or accept the cosmetic loss.
