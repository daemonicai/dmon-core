...
- **8.1 — e2e walk surfaced a real launch defect (now fixed).** Running `dmon` against a real `./Dmon.cs` crashed with `MSB4025: The project file could not be loaded`. Root cause: `CoreProcessManager.BuildFileBasedProgramAsync` invoked `dotnet build <Dmon.cs> --tl:off`; the trailing `--tl:off` CLI flag is positionally consumed by `dotnet build`, defeating file-based-app dispatch so the SDK parses `Dmon.cs` as an XML project file. Reproduced directly: `dotnet build Dmon.cs --tl:off` → MSB4025; `dotnet build Dmon.cs` and `MSBUILDTERMINALLOGGER=off dotnet build Dmon.cs` both dispatch correctly as file-based apps.
  - **Fix:** dropped the `--tl:off` arg; set `psi.Environment["MSBUILDTERMINALLOGGER"] = "off"` instead — keeps captured build output free of terminal-logger noise without disturbing dispatch. Added an anti-revert comment at the call site.
  - **Test-divergence fix (the bug had been masked):** `FileBasedProgramLaunchTests` re-implemented the build command in a private `RunDotnetBuildAsync` helper that never had `--tl:off`, so it stayed green while production was broken. Exposed the production `CoreProcessManager.BuildFileBasedProgramAsync` via `[assembly: InternalsVisibleTo("Dmon.Core.Tests")]` (matched the existing `AssemblyInfo.cs` attribute style) + a `Dmon.Core.Tests → Dmon.Runtime` `ProjectReference` (no transitive path existed; no cycle — Runtime → Protocol only), rewired both call sites to the real method, and deleted the divergent helper. A future `--tl:off`-class regression now fails these tests.
  - **Second finding (env, not a code bug):** a hand-authored `Dmon.cs` pinning `dmoncore@0.2.*` cannot restore from nuget.org (only `0.1.0` is public, pre-release) — it needs a `nuget.config` pointing at the local `build/core-feed` (which packs `dmoncore`, `Dmon.SampleExtension`, and the contract packages at `0.2.0`). The empty-dir/prebuilt-default path needs no restore (`dotnet exec build/dmoncore/dmoncore.dll`).
  - **Verification:** **Leg 1 confirmed by hand** — empty dir → prebuilt default → a live model turn replied. The composed-extension and `/reload` mechanics are covered by the now-real automated tests (`FileBasedProgramLaunchTests` 2/2 on first-build + rebuild-after-edit, driving the production build command; `ComposedCoreWireTests` extension tools at startup with no `extensionLoaded` event).
- **8.2 — full gates green:** `make build` 0 warnings / 0 errors; `make test` 0 failures across all 10 suites (Core 545, Terminal 159, Gateway 143, BuiltinTools 102, Protocol 100, Memory 51, Omlx 41, Providers 32, Extensions 26, Runtime 15; 2 pre-existing skips); `openspec validate composition-root-hosting --strict` valid.
- **Files:** `src/Dmon.Runtime/CoreProcessManager.cs`, `src/Dmon.Runtime/AssemblyInfo.cs`, `test/Dmon.Core.Tests/Composition/FileBasedProgramLaunchTests.cs`, `test/Dmon.Core.Tests/Dmon.Core.Tests.csproj`.

## DONE

- **All groups 1–8 complete.** Change ready for `/opsx:archive` once merged. **Task 1.3** (entry-point-less library / no `Main`) was completed back in G1.3 (`OutputType=Library` + `GenerateRuntimeConfigurationFiles=false`).
- **Nits / deferred:**
