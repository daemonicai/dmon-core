...
- Migrate the **Gateway's two-step create/load handshake** (`DriveSessionHandshakeAsync` + `ReadCorrelatedResultAsync`) onto the shared framing seam via a lossless, non-read-ahead transport-level correlated request (`IRpcTransport` + `await foreach`/break — **not** the continuous-pump `IRpcClient`, which would read-ahead into the lossy broadcast and corrupt the ADR-014 seq stream; see design D7). **Out of scope:** `SessionHandler`'s ongoing RPC loop (ADR-014 seq buffer, permission correlation) stays as-is.
