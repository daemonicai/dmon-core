...
- [x] 2.3 At archive, confirm the Purpose direct-edits survived the delta sync (re-apply if `openspec archive` regenerated `provider-extension`/`daemon-composition-root` from source). Record the outcome in DEVLOG (resolves design D3). **Outcome (2026-07-14): survived, no re-apply needed.** `openspec archive` merges requirement-level deltas in place (provider-extension `ProviderName` req synced to current-provider examples) without regenerating the file, so both `## Purpose` direct-edits stand: provider-extension L8 = `IToolExtension`/no-oMLX; daemon-composition-root L3 = "first-line mlx, escalation mlx" (untouched — it had no delta). Post-archive sweep of the five synced standing specs found zero residual `oMLX`/`IDaemonExtension`. Design D3 resolved: Purpose-prose direct edits are safe alongside delta syncs.
