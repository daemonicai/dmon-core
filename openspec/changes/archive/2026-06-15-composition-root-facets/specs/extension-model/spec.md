...
- FROM: `### Requirement: Dmon.Extensions exports IDmonMiddleware and DmonMiddlewareAttribute`
- TO: `### Requirement: Dmon.Abstractions exports the tool, middleware, and facet contracts`
...
**Reason**: The contract rename (`IDmonExtension`→`IToolExtension`) and the deletion of `Dmon.Extensions` are deliberate breaking SDK changes; back-compat is explicitly not required (no production deployments). A binary-compatibility guarantee for extensions compiled against the old contract is therefore obsolete and contradicts the rename.
**Migration**: Recompile out-of-tree extensions against `Dmon.Abstractions`, replacing `IDmonExtension` with `IToolExtension`. There is no binary-compatible path; this is a clean break.

### Requirement: Daemon.Extensions references Daemon.Protocol
**Reason**: Obsolete. The `Daemon.Extensions` project (and its successor `Dmon.Extensions`) is deleted by this change; all author-facing contracts collapse into `Dmon.Abstractions`, which already carries `Dmon.Protocol`. The historic `Daemon.*` naming predates the project rename and no longer maps to a live project.
**Migration**: No action. The `IToolExtension` contract lives in `Dmon.Abstractions`, which references `Dmon.Protocol`, supplying `IPermissionSettings`/`PermissionResult`/`ToolConfirmRequest`.
