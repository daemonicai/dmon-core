...
### Block 4 — tasks 7.1, 8.1–8.3, 9.1, 9.2 — Daemon.cs oMLX rewire + privacy confirm + gates ✅
- **Daemon.cs rewired** (only file changed for 8.x): `builder.UseOmlx()` (non-hijacking) → `UseTriage(sp => sp.OmlxClient(firstLineModel))` → `AddEscalation(sp => sp.OmlxClient(escalationModel))` → `AddEgress(egress)` (eager Gemini). Old Ollama-classifier + OpenAI-reasoner `IChatClient` constructions removed. `#:project` refs: +`Dmon.Providers.Omlx`, −`Dmon.Providers.OpenAI`, −`Dmon.Providers.Ollama` (verified Ollama only used by the old e2b classifier; `Daemon.Routing.csproj` doesn't reference OllamaSharp). Removed `using OllamaSharp;`/OpenAI usings.
- **Config (8.2):** dropped `DMON_E2B_URL`/`DMON_E2B_MODEL`/`DMON_REASONER_URL`/`DMON_REASONER_MODEL`; added `DMON_FIRSTLINE_MODEL` (default `gemma-4-e4b-it-qat-OptiQ-4bit`) + `DMON_ESCALATION_MODEL` (default `gemma-4-26B-a4b-it-qat-OptiQ-4bit`) — **user's exact oMLX ids** (orchestrator corrected the architect's simplified defaults); `DMON_EGRESS_MODEL` default → `gemini-3.1-flash-lite`. `OMLX_BASE_URL` sourced inside `UseOmlx()`/`OmlxConfig.FromEnvironment()`, not read in Daemon.cs.
- **7.1 confirm:** existing block-3 tests cover all four invariants; reviewer found the first-line *manifest tools* weren't asserted (only egress path) → added `PersonalTurn_FirstLineManifestContainsPersonalToolAndThinkHarder_NotWorldTool` (captures the first-line backend's `ChatOptions.Tools`: personal tool + think_harder present, world tool absent). Daemon.Routing.Tests now **33/33**.
- **Gates:** `dotnet build daemon/Daemon.cs -c Release` 0/0 (Daemon.cs is NOT in Everything.slnx — this explicit build is the only automatable proof the rewire compiles); `make build` 0-warn; `env -u MEKO_API_KEY make test` green (33/33 routing); `openspec validate --strict` valid.
- Reviewer: **Approve** (block-4 diff), no blockers; cosmetic whitespace nit left.

## Outstanding before archive
- **LIVE oMLX escalation verification (human-in-the-loop, task 8.1's runtime behavior):** unit tests assert the router's detect/strip/handoff given a *synthesised* think_harder pair; they do NOT exercise the real `FunctionInvokingChatClient` `Terminate` loop-stop against live gemma models. 8.1's *code* is ticked (wiring complete + compiles); the end-to-end escalation behavior needs a manual run against the oMLX app + models (recipe handed to the user). Confirm before `/opsx:archive`.
- **9.3** is unticked by design — it's the **archive-time** spec fold (`/opsx:archive` folds the `triage-routing` + `omlx-provider` deltas into `openspec/specs/`).
