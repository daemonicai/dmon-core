...
- [ ] 5.1 Implement session directory discovery (walk up from CWD for `.daemon/`, fallback to `~/.daemon/`)
- [ ] 5.2 Implement `SessionStore` — create, load, list sessions
- [ ] 5.3 Implement `messages.jsonl` append writer
- [ ] 5.4 Implement attachment threshold logic using `IConfiguration` (`Daemon:Session:AttachmentThresholdBytes`, default 1024)
- [ ] 5.5 Implement `meta.json` read/write
- [ ] 5.6 Implement SQLite global index (`sessions.db`) — upsert on create/modify, rebuild-from-scan on corruption
- [ ] 5.7 Implement `session.fork` — `cp -r` source directory to new session id, truncate the *new copy's* `messages.jsonl` after the line containing `entryId` (source is never mutated), retain `attachments/` referenced by retained messages, rewrite `meta.json` with new id and `parentSession`/`forkEntryId`
- [ ] 5.8 Implement `session.clone` — full directory copy with new id
- [ ] 5.9 Implement `CompactionMessage` appending and reader-side compaction marker handling
- [ ] 5.10 Write unit tests for session discovery, append, fork, clone, compaction
