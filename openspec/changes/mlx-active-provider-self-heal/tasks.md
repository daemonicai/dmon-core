...
- [x] 2.1 Simplify `MlxClientExtensions.MlxClient(key)` to resolve the keyed `MlxProviderExtension`, build the `ProviderConfig`, and return `((MlxProviderFactory)ext.CreateFactory()).CreateAsync(...)` — removing its duplicate up-front `EnsureRunningAsync` call and its own `EnsureRunningChatClient` wrap.
- [x] 2.2 Confirm the keyed registration/warm/stop seams (`AddMlxFirstline`/`AddMlxEscalation`, `EscalationWarmingService`, `MlxRuntimeKeys`) are untouched.
...
- [x] 3.4 Add coverage that `MlxClient(key)` returns a self-healing client sourced from the factory and no longer double-invokes/double-wraps the self-heal.
