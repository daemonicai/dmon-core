...
- [x] 2.1 In `ApiKeyAuthExtensions.InvokeAsync`, exempt exactly `GET /api/auth/google/login` and `GET /api/auth/google/callback` from the `X-Api-Key` check using an exact, closed allow-list (segment-boundary match), leaving every other `/api/*` path default-deny.

## 3. Callback state/PKCE compensating control

- [x] 3.1 Confirm the callback rejects an unknown `state` with `400 invalid_state` and does not exchange the code (existing behaviour via `OAuth2StateStore.GetVerifier`); keep the single-use (`TryRemove`) semantics so a `state` cannot be replayed. Adjust only if the exemption changed the flow.

## 4. Tests

- [x] 4.1 Test: `GET /api/auth/google/callback` with **no** `X-Api-Key` is NOT rejected with 401 by the middleware (reaches the handler; unknown state → 400 `invalid_state`).
- [x] 4.2 Test: `GET /api/auth/google/login` with **no** `X-Api-Key` is NOT rejected with 401 (reaches the handler / issues the Google redirect).
- [x] 4.3 Test: a non-exempt `/api/*` path (e.g. `GET /api/status`, and a decoy `/api/auth/other`) with no `X-Api-Key` still returns 401.
...
- [x] 4.5 Test: callback with an unknown/consumed `state` returns 400 `invalid_state` and performs no token exchange (state is single-use / non-replayable).

## 5. Gates

- [x] 5.1 `make build` clean (no warnings; `TreatWarningsAsErrors`).
- [x] 5.2 `env -u MEKO_API_KEY make test` green (new tests + all existing).
- [x] 5.3 `openspec validate dmail-oauth-hardening --strict` passes.
