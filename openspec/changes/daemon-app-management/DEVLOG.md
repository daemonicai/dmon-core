...
- **Build gotcha:** `swift build` cache can report "Build complete (0.11s)" without recompiling after edits, and SourceKit diagnostics can be stale mid-edit. To verify a Swift block for real, `rm -rf daemon/Daemon.App/.build` before `make daemon-app`.
