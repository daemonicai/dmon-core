...
**D2 — Sequencing and replay are gateway-layer over `messages.jsonl`.** The gateway assigns per-session `seq` and uses the append-only log (ADR-004) as the durable replay source; no log-format change. Reattach uses **subscribe-then-replay**: subscribe live into a buffer, replay `(lastSeq, headSeq]` from the log, flush the buffer deduping by `seq`. *Alternative — have the core emit `seq` — rejected: keeps ADR-003 and the core untouched; sequencing is a transport concern.*
