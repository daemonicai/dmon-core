...
- [x] 12.1 Fix the internal-constructor DI failures so the host boots. In `frontends/Dmon.Network/DeviceKeys/DeviceKeyStoreWatcher.cs` make the single constructor `public` (the class stays `internal sealed`). In `frontends/Dmon.Network/DeviceKeys/LastSeenWriter.cs` make **only** the `(NetworkDeviceKeyPaths paths, …)` constructor `public`; leave the `(string? path, …)` constructor and `CreateNoOp()` `internal` (avoids an ambiguous-constructor DI selection; test access is via `InternalsVisibleTo`). No other ctor or registration changes.
- [x] 12.2 Add a host DI-graph validation regression test. Extract the service registrations from `Program.cs` (current lines 72–107) into a testable `static IServiceCollection AddNetworkHostServices(this IServiceCollection, NetworkDeviceKeyPaths, DeviceKeySet)` in a new `frontends/Dmon.Network/NetworkHostServices.cs`, and have `Program.cs` call it. Add a test in `test/Dmon.Network.Tests` that registers `Configure<NetworkOptions>` + logging, calls `AddNetworkHostServices`, builds the provider with `ValidateOnBuild`/`ValidateScopes` true, **and explicitly resolves `IEnumerable<IHostedService>` plus `NetworkConnectionEndpoint` and `LastSeenWriter`** to force construction (build-time validation alone does not construct hosted/instance/factory services, so the explicit resolves are what catch this bug).
- [x] 12.3 Gates: `make build` clean (`TreatWarningsAsErrors`), `env -u MEKO_API_KEY make test` green (new DI-validation test + all existing `Dmon.Network.Tests`), `openspec validate gateway-packaging --strict`. Hand off for the orchestrator to re-run the 10.4 human-verify (launch via `make network`, confirm dmonium's **Network** row goes green OOTB).
- [x] 12.4 Make `make network` reliably reinstall the freshly-built binary. The pinned `--version 0.1.0` makes `dotnet tool update` a no-op when 0.1.0 is already installed (NuGet treats the version as immutable/cached), so a re-run after a code change keeps the STALE tool — violating task 5.1's "update if already installed". Fix the `network` target to force a fresh install of the just-packed nupkg (uninstall-if-present + clear the package's NuGet cache entry + install, or an equivalent that guarantees the current code lands at `~/.dotnet/tools/ndmon`). Verify by a bounded launch that `~/.dotnet/tools/ndmon` boots without the DI crash after `make network`.
