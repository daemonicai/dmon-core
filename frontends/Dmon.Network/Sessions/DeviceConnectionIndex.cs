...
/// <see cref="GetConnections"/> and <see cref="INetworkConnection.Abort"/>s each entry.
///
...
internal sealed class DeviceConnectionIndex
{
    ...
    // the WebSocketNetworkConnection instances used in production.
    private readonly ConcurrentDictionary<string, ConcurrentDictionary<INetworkConnection, byte>> _index = new();

    /// <summary>
    /// Records <paramref name="connection"/> as live under <paramref name="keyId"/>.
    /// </summary>
    public void Add(string keyId, INetworkConnection connection)
    {
        ConcurrentDictionary<INetworkConnection, byte> bucket =
            _index.GetOrAdd(keyId, _ => new ConcurrentDictionary<INetworkConnection, byte>(ReferenceEqualityComparer.Instance));
    ...
    /// </summary>
    public void Remove(string keyId, INetworkConnection connection)
    {
        if (!_index.TryGetValue(keyId, out ConcurrentDictionary<INetworkConnection, byte>? bucket))
            return;
        ...
        if (bucket.IsEmpty)
            _index.TryRemove(new KeyValuePair<string, ConcurrentDictionary<INetworkConnection, byte>>(keyId, bucket));
    }
    ...
    /// </summary>
    public IReadOnlyCollection<INetworkConnection> GetConnections(string keyId)
    {
        if (!_index.TryGetValue(keyId, out ConcurrentDictionary<INetworkConnection, byte>? bucket))
            return [];
