...
**Superseded in part:** **Decision 1 only** — the `home/` bucket — by **ADR-038** (2026-09-08). `dmon-home` now lives in its own repository, `daemonicai/dmon-home`, and `home/` no longer exists here; D1's amendments to ADR-025 D2 and ADR-028 D1 are reverted with it. **Decisions 2, 3, 4 and 5 are untouched and still binding**, and D3's obligation to retire `dmonium` only at parity survives the split as a cross-repo obligation ADR-038 D2 restates. This ADR remains the record of why `dmon-home` exists and what it is; ADR-038 records only why it left.
