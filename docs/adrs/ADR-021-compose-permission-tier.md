...
4. **`compose` gates the *agent*, not reload in general.** A human who edits a composition root and runs `/reload` is exercising their own authority (exactly "you committed the project"); no `compose` prompt. The tier targets the agent changing its own composition. The fully-autonomous case — agent edits *and* agent reloads with no human in the loop — is precisely where the gate is load-bearing (Decision 6).
...
- **C. Attributing an edit to agent vs human at reload.** The reliable signal (the agent's own edit-tool call within the session) vs the fallback (a composition diff with no corresponding agent action). How robust the attribution must be, and the safe default when ambiguous (treat as agent-initiated → gate).
